...
source <(curl -s https://raw.githubusercontent.com/jerlysonsantos/bumblevim-requiem/main/config.sh)
source <(curl -s https://raw.githubusercontent.com/jerlysonsantos/bumblevim-requiem/main/themes.sh)
...
__init__() {
	...
	select option in nodejs python quit; do
		case $option in 
			nodejs)
                $CURRENT_LANG = "nodejs"
				validate_node
